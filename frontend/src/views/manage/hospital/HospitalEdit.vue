...
export default {
  ...
  methods: {
    ...
    setFormValues ({...hospital}) {
      ...
      let fields = ['hospitalName', 'hospitalSex', 'hospitalImg', 'hospitalTitle', 'teachTitle', 'hospitalAdministrative', 'hospitalDegree', 'hospitalForte', 'hospitalAbout', 'hospitalId', 'officesId']
      let obj = {}
      Object.keys(hospital).forEach((key) => {
        ...
        // if ((key === 'hospitalId' || key === 'officesId') && hospital[key] != null) {
        //   hospital[key] = hospital[key].toString()
        // }
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key)
          obj[key] = hospital[key]
        }
        if (key === 'hospitalId' && hospital['hospitalId'] != null && hospital['hospitalName'] != null) {
          this.fetchUser(hospital['hospitalName'])
          this.selectOfficeList(hospital['hospitalId'])
        }
      })
      setTimeout(() => {
        this.form.setFieldsValue(obj)
