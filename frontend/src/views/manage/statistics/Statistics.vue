...
export default {
  ...
  methods: {
    ...
    selectOrderDays (date) {
      date = date + '-01'
      this.loading = true
      this.$get(`/cos/pay-record/selectStatisticsByMonth`, {
        date
      }).then((r) => {
        this.series1 = []
        this.series2 = []
        console.log(r.data)
        if (r.data.numList && r.data.numList.length !== 0) {
          this.chartOptions1.xaxis.categories = Array.from(r.data.numList, ({name}) => name)
          r.data.numList.forEach(e => {
            let value = { name: e.name, data: [e.value] }
            this.series1.push(value)
          })
        }
        if (r.data.priceList && r.data.priceList.length !== 0) {
          this.chartOptions2.xaxis.categories = Array.from(r.data.priceList, ({name}) => name)
          r.data.priceList.forEach(e => {
            let value = { name: e.name, data: [e.value] }
