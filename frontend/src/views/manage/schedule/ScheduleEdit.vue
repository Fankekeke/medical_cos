  ...
  <a-modal v-model="show" title="修改排班" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        ...
        <a-col :span="12">
          <a-form-item label='排班性别' v-bind="formItemLayout">
            <a-select v-decorator="[
              'sex',
              { rules: [{ required: true, message: '请输入排班性别!' }] }
              ]">
  ...
  methods: {
    ...
    setFormValues ({...schedule}) {
      ...
      Object.keys(schedule).forEach((key) => {
        ...
        if (key === 'deptId' && schedule['deptId'] != null && schedule['hospitalName'] != null) {
          this.fetchUser(schedule['hospitalName'])
        }
      })
